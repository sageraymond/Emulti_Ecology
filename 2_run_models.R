...
guide <- readRDS("builds/batchmods_nov/model_guide.Rds")
...
if(rerun_all){
  file.remove(list.files("builds/batchmods_nov/models/", full.names = T))
}
...
#nrow(working_guide) # these are the models that didn't run
 ...
 #  TRY DIFFERENT APPROACH
   ...
   #        Scale variables inside sub_dat 
        sub_dat[, `:=` (
          Apple_scaled = as.vector(scale(Apples)),
          Berries_scaled = as.vector(scale(Berries)),
          Birdseed_scaled = as.vector(scale(Birdseed)),
          Vegetation_scaled = as.vector(scale(Vegetation)),
          Nat_Prey_scaled = as.vector(scale(Nat_Prey)),
          Garbage_scaled = as.vector(scale(Garbage)),
          Unk_Anthro_scaled = as.vector(scale(Unk_Anthro)),
          proplowincome_scaled = as.vector(scale(proplowincome)),
          prophighincome_scaled = as.vector(scale(prophighincome)),
          Dogs_scaled = as.vector(scale(Dogs)),
          ...
          cover_Nat_25m_scaled = as.vector(scale(cover_Nat_25m)),
          cover_Anth_25m_scaled = as.vector(scale(cover_Anth_25m)),
          cover_Nat_50m_scaled = as.vector(scale(cover_Nat_50m)),
          cover_Anth_50m_scaled = as.vector(scale(cover_Anth_50m)),
          cover_Nat_250m_scaled = as.vector(scale(cover_Nat_250m)),
          cover_Anth_250m_scaled = as.vector(scale(cover_Anth_250m)),
          bldg_dens_25mH_scaled = as.vector(scale(bldg_dens_25mH)),
          bldg_dens_50mH_scaled = as.vector(scale(bldg_dens_50mH)),
          bldg_dens_250mH_scaled = as.vector(scale(bldg_dens_250mH)),
          road_dens_25mH_scaled = as.vector(scale(road_dens_25mH)),
          road_dens_50mH_scaled = as.vector(scale(road_dens_50mH)),
          road_dens_250mH_scaled = as.vector(scale(road_dens_250mH)),
          urban_index_pca_scaled = as.vector(scale(urban_index_pca)),
          ...
          Prop.NoCertDiplomadegree_scaled = as.vector(scale(Prop.NoCertDiplomadegree)),
          Prop.Over65_scaled = as.vector(scale(Prop.Over65)),
          Prop.Under15_scaled = as.vector(scale(Prop.Under15))
        )]
...
# OK. 9 failed. I can live with this no problem
