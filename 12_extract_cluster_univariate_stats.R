...
master_guide <- readRDS("builds/batchmods_nov25/model_guide.Rds")
...
master_guide_wide[model_complexity_comparison_ID == "model_complexity_id_18"]
master_guide_wide

master_guide_wide[model_complexity_comparison_ID == "model_complexity_id_37"]
...
comps.dt[is.na(null_uni_chisq)] #logically those 4 dudes that didnt converge
# Ideally 0 rows
...
tidy_glmmTMB <- function(m) {
  ...
  adj_r2 <- tryCatch({
    y <- model.frame(m)[,1]                 # response variable
    y_hat <- predict(m, type = "response")  # fitted values
    rss <- sum((y - y_hat)^2)
    tss <- sum((y - mean(y))^2)
    r2 <- 1 - rss/tss
    
    p <- length(fixef(m)$cond) - 1  # number of fixed effects excluding intercept
    r2_adj <- 1 - (1 - r2) * (n_obs - 1)/(n_obs - p - 1)
    r2_adj
  }, error = function(e) NA)
...
saveRDS(master_guide_wide.mrg1, "builds/batchmods_nov25/model_guide_with_model_stats.Rds")
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ---------------------------------
...
#write.csv(master_guide_wide.mrg2, "data/AllUnivariateClusterNov25.csv")
